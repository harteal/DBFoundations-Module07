...
-- 11/23/2024, Alex Harteloo, Script Created 
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_AlexHarteloo')
	 Begin 
	  Alter Database [Assignment07DB_AlexHarteloo] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_AlexHarteloo;
	 End
	Create Database Assignment07DB_AlexHarteloo;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_AlexHarteloo;
...
go
create VIEW [vProductDateCount]
AS
SELECT
  vP.ProductName
,[InventoryDate] = DATENAME(MONTH,CONVERT(datetime, vI.InventoryDate)) + ', ' + DATENAME(YEAR, CONVERT(datetime, vI.InventoryDate))
...
,[PreviousMonthCount] = 
	lag(vI.Count, 1, 0) 
	OVER(PARTITION BY vP.ProductName ORDER BY InventoryDate)
from vProducts as vP 
INNER JOIN vInventories as vI
	ON vP.ProductID = vI.ProductID
go
...
SELECT
 vP.ProductName
,[InventoryDate] = DATENAME(MONTH,CONVERT(datetime, vI.InventoryDate)) + ', ' + DATENAME(YEAR, CONVERT(datetime, vI.InventoryDate))
,[CurrentMonthCount] = vI.Count
,[PreviousMonthCount] = 
	lag(vI.Count, 1, 0) 
	OVER(PARTITION BY vP.ProductName ORDER BY InventoryDate)
--,[MonthTrendKPI] = Case
	--when PreviousMonthCount > PreviousMonthCount then 1
